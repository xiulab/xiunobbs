...
function sms_send($tomobile, $message) {
	return sms_tencent_send($tomobile, $message);
}

function sms_tencent_send($tomobile, $message) {
	...
	// 短信应用SDK AppID
	$appid = 1400067450; // 1400开头
	
	// 短信应用SDK AppKey
	$appkey = "4a41a0bcf465f41da75b2d9ae9897a5e";
	
	// 需要发送短信的手机号码
	$phoneNumber = 15600900902;
	
	// 短信模板ID，需要在短信应用中申请
	//$templateId = 7839;  // NOTE: 这里的模板ID`7839`只是一个示例，真实的模板ID需要在短信控制台中申请
	
	$smsSign = "修罗开源"; // NOTE: 这里的签名只是示例，请使用真实的已申请的签名，签名参数使用的是`签名内容`，而不是`签名ID`
