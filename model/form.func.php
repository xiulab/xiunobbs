...
function form_radio($name, $arr, $checked = 0) {
	...
	foreach((array)$arr as $k=>$v) {
		...
		$s .= "<label class=\"custom-input custom-radio\"><input type=\"radio\" name=\"$name\" value=\"$k\"$add /><span class=\"c-indicator\"></span>$v</label> &nbsp; \r\n";
	}
	return $s;
}
...
function form_checkbox($name, $checked = 0, $txt = '') {
	...
	$s = "<label class=\"custom-input custom-checkbox\"><input type=\"checkbox\" name=\"$name\" value=\"1\" $add /><span class=\"c-indicator\"></span>$txt</label>";
	return $s;
}
